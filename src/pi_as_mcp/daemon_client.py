...
class DaemonClient:
    ...
    def request(self, command: str, *, request_timeout_seconds: int = 30, **params: Any) -> dict[str, Any]:
        ...
        # backoff. A failure after connect is NOT retried: commands like
        # delegate/reply are not idempotent and may already be executing.
        try:
            chunks = self._send(payload, request_timeout_seconds)
        except _DaemonConnectError:
            self.start_daemon()
            try:
                chunks = self._send_with_connect_retries(payload, request_timeout_seconds)
